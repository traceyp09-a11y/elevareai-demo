...
export default function DashboardSales() {
  ...
  const showAllKPIs = (e?: React.MouseEvent) => {
    ...
    const kpiList = salesKPINames.map((name, i) => `${i + 1}. ${name}`).join('\n');
    alert(`Sales Department KPIs:\n\n${kpiList}`);
  };
          ...
          >
            ...
            {expandedKPI === kpi.key && (
              <div className="mt-4 pt-4 border-t border-gray-700">
                <div className="mb-3">
                  <h4 className="text-sm font-semibold text-amber-400 mb-1">Calculation Formula</h4>
                  <p className="text-xs text-gray-300 font-mono bg-gray-800/50 p-2 rounded">
                    {kpi.formula}
                  </p>
                </div>
                <div>
                  <h4 className="text-sm font-semibold text-amber-400 mb-1">Data Points</h4>
