...
export default function PainPointsFinance() {
  ...
  const fetchPainPoints = async () => {
    try {
      const response = await axios.get<PainPointsResponse>('http://localhost:3001/api/finance/pain-points');
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white p-8">
        <div className="flex items-center justify-center h-64">
          <div className="text-xl">Loading Finance Pain Points...</div>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white p-8">
      ...
      <div className="mb-8">
        <h1 className="text-4xl font-bold bg-gradient-to-r from-green-400 to-emerald-500 bg-clip-text text-transparent mb-2">
          Finance Pain Points Analysis
        </h1>
        <p className="text-gray-400">
          Top 10 critical financial challenges requiring C-Suite attention and investment
        </p>
      </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <span className="ml-2 font-semibold text-blue-400">{painPoint.timeline}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {expandedCard === painPoint.id && (
              <div className="mt-6 pt-6 border-t border-gray-700">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Affected Metric */}
                  <div>
                    <h4 className="text-sm font-semibold text-green-400 mb-2">Affected Metric</h4>
                    <p className="text-gray-300">{painPoint.affected_metric}</p>
                  </div>

                  {/* Responsible Department */}
                  <div>
                    <h4 className="text-sm font-semibold text-green-400 mb-2">Responsible</h4>
